...
20. As an image editor user, I want a cursor pixel inspector, so that I can read exact pixel coordinates and colors.
21. As an image editor user, I want the pixel inspector to show x and y coordinates, so that I know exactly where I am inspecting.
22. As an image editor user, I want the pixel inspector to show original hex color, so that I can understand the source pixel.
23. As an image editor user, I want the pixel inspector to show processed hex color, so that I can understand the palette-mapped result.
24. As an image editor user, I want slide compare's pixel inspector to show both original and processed values regardless of divider position, so that I can compare exact pixels quickly.
25. As an image editor user, I want the pixel inspector to hide when the cursor leaves the preview, so that it does not clutter the interface.
26. As an image editor user, I want the pixel inspector to degrade gracefully when processed output is not ready, so that the UI remains useful while processing.
...
- If end-to-end UI tests are added later, they should verify that zooming, panning, and loupe are visible behaviors, not implementation details.
